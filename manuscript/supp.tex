...
%---------------------------------------------------------------
\section{Data Quality Assessment: Visualizations, Statistics, and Findings}


This section presents illustrative examples of the chart types and analytical 
...
below demonstrate both the visualization methods and the interpretive framework 
...
durations---by NYC Agency. These charts serve two diagnostic purposes. First, 
they identify whether an anomaly concentrates within a single agency, suggesting 
an agency-specific data entry error or system configuration issue requiring 
targeted correction. Second, they reveal whether anomalies distribute 
proportionally across agencies mirroring the overall 311 Service Request (SR) 
volume distribution, indicating a systemic problem affecting the entire dataset 
rather than isolated to particular agencies.
...
or magnitude patterns). Each visualization type supports specific analytical 
...
\subsection{Service Request Status and Closure Analysis}
...
is shown to primarily exist within the Department of Transportation (DOT)---a 
full 77\%, with smaller anomalies occurring within the Department of
...
The charts can also reveal trends in an anomaly. As noted in the reference manuscript,
...
software program that seems to close selected SRs exactly at midnight. However, 
as shown in Figure~\ref{fig:closed-exact-midnight-cy} the chart of the 
yearly distribution of midnight \texttt{closed\_date)}(s), 
...
Supplementing that discovery is Figure~\ref{fig:pareto-closed-midnight}, the 
Pareto chart of the midnight anomaly by Agency. The chart shows that 
the majority of the SRs closed-at-midnight occur within just two Agencies, DSNY 
and DOB, which collectively are responsible for 97.6\% of such anomalies.
...
\subsection{Service Request Positive Duration Analysis}
...
In Figure~\ref{fig:boxplot-large-positive-durations} the box plot  shows duration 
...
magnitude---15.3 million observations (94\% of the dataset). Figure~\ref{fig:histogram_city_wide} 
...
significant ($D = 0.027$, $p < 2.2 \times 10^{-16}$), which decisively rejects the 
null hypothesis of unimodality. And indeed Figure~\ref{fig:bimodal_hist} visually
confirms that; indeed it depicts a bimodal distribution, which would seem
to require further analysis and investigation.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.9\textwidth]{positve_duration_distribution_log.pdf}
\caption{Distribution of SRs with Positive Durations: City-wide (log scale).}
...
it located the local minimum between the two peaks. This valley-based 
approach yielded a threshold of 0.432 days (10.4 hours), which provides a 
criterion for classifying service requests into two categories: rapid-resolution 
($< 0.432$ days duration) and standard work orders ($\geq 0.432$ days duration).
...
days and mean of 47.3 days. This near-perfect 50/50 split masks dramatically different
operational models across city agencies---a clear example of Simpson's Paradox, 
where aggregate statistics obscure the true underlying structure when different 
populations are inappropriately combined. 
...
accounting for 87.2\% of all fast closures (6.69M out of 7.66M requests). 
Critically, 96.8\% of NYPD's SRs close within 10.4 hours, reflecting rapid patrol responses 
to quality-of-life complaints. In stark contrast, infrastructure 
and regulatory agencies operate primarily in standard mode: HPD (96.1\% standard), 
DSNY (92.9\% standard), DOB (91.1\% standard), and DPR (90.1\% standard). It
seems reasonable to assume that the nature of the SRs for these agencies require 
more physical service delivery, construction permits, code 
enforcement, or ongoing case investigation, thus creating longer SR durartions. 

This bimodal structure has significant implications for data quality assessment and 
...
represents neither reality. Performance metrics, anomaly 
...
\begin{tablenotes}
\small
\item Note: Rapid-resolution is defined as an SR duration of $< 0.432$ days (10.4 hours); 
standard work orders have a duration of $\geq 0.432$ days. 
\end{tablenotes}
\end{table}
...
\begin{tablenotes}
\small
\item Note: Agencies ordered by total volume. NYPD accounts for 87.2\% of all 
rapid-resolution requests (6.69M of 7.66M). Percentages show within-agency 
distribution across modes.
\end{tablenotes}
\end{table}


Visualizing how the durations of these two operational modes requires separating
the durations of the NYPD versus all other Agencies. Figure~\ref{fig:histogram_NYPD}
show NYPD-only positive distributions. Of note is that the Median and Mean lines 
are somewhat separated, indicating right skewness in NYPD durations; a 
small number of large durations have an outsized effect on the Mean.
...
In constrast to the NYPD-only durations is the aggregation of all other NYC Agencies.
Here Figure~\ref{fig:histogram_other}  reveals a broader range of durations, with
an exceptional number of large to very large durations in the \textgreater{}1000 
days (note that the x-axis is logarithmic scale). 
 how the durations of these two operational modes requires separating
the durations of the NYPD versus all other Agencies. Figure~\ref{fig:histogram_NYPD}
show NYPD-only positive distributions. 
...
Finally, the clear picture emerges as the two distributions (NYPD vs. all other Agencies) are 
superimposed on the same chart.  Figure`\ref{fig:histogram_combined} clearly shows
the bimodal distribution likely reflecting different operational protocols for the two
categories. Keep in mind that the NYPD is the single largest user of the 311 non-emergency
service, representing 43\% of all SRs.
...
showing negative durations --- where the \texttt{closed\_date} occurs b
efore the \texttt{created\_date} resulting 
in a nonsensical negative duration. in Figure~\ref{fig:boxplot-negative-durations} , 
the box plot shows DOT to be the largest offender. However, the mean 
negative duration is far greater for the 
...
are identical, to the second. Indeed there are 387{,}329 such impossible duration SRs. 
...
small, e. g. just a few seconds, and are highly suspicious. These are classified as near-zero
durations. 

As noted in the referenced manuscript, a quantitative analysis 
revealed a boundary of 28 seconds as
an outlier (LogNormal\_$3\sigma$). The results of this analysis are shown sorted
by the detection threshold method in Figure~\ref{fig:zero-duration-pareto-combo} 
below. Using this value reveals 14$,$741 suspicious durations. Also below, 
Figure~\ref{fig:zero-duration-pareto-combo} and Figure~\ref{fig:zero-duration-pareto-combo} 
charts that capture the analysis of this suspicious boundary. These two charts 
help visualize the overall distribution of low duration SRs from 2---90 seconds; one 
showing cumulative and one showing durations by count. The cumulative distribution 
...
the \texttt{jds\_datacleansings.R} program, there will be 78 charts in .PDF format in 
...
in the above tree structure. Compare the charts to those in the original manuscript

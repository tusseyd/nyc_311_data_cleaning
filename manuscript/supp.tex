...
%
% This document provides reproducibility details, extended analyses,
% and supporting figures for the main manuscript.
%=================================================================
...
durations---by NYC agencies. As such, these charts serve a diagnostic purpose: 
...
($D = 0.027$, $p < 2.2 \times 10^{-16}$), an effectively zero p-value that 
...
\caption{Overall Distribution of SRs with Positive Durations (Log Scale).}
...
the nature of the SRs for these agencies requires more physical 
...
constitute the vast majority of standard durations. A deeper analysis can help
discover the underlying, distinct distributions. Visualizing how the 
durations of these two operational modes requires separating the 
durations of the NYPD versus all other Agencies. 
Figure~\ref{fig:histogram_NYPD} shows NYPD-only positive 
...
\caption{NYPD-Only Distribution of SRs with Positive Durations (Log Scale).}
\label{fig:histogram_NYPD}
\end{figure}


In contrast to the NYPD-only durations, the aggregation of all 
other NYC agencies, shown in Figure~\ref{fig:histogram_other}, reveals 
a broader range of durations, with an exceptional number of very large, 
almost extreme, durations exceeding 1,000 days (note that the x-axis is a 
logarithmic scale). 
...
\caption{All Non-NYPD Agencies: Distribution of SRs with Positive Durations (Log Scale).}
...
Thus, it is not unexpected that one agency has an outsized 
...
\caption{Combined Duration Profiles: NYPD and All Other Agencies (Log scale).}
...
durations. As noted in the referenced document, a quantitative analysis 
revealed a boundary of 28 seconds as an outlier based on the 
(LogNormal\_$3\sigma$) computed on the log-transformed 
duration values and then back-transformed to the original scale. 
Using this value reveals 14{,}741 suspicious durations. The referenced 
document also contains rationale for the use of this outlier methodology.


Figure~\ref{fig:duration-histogram} visualizes this suspicious 
boundary. It shows the overall 
...
\end{itemize}
The DOIs listed above correspond to the archived dataset versions used in this submission.
Future dataset updates, if any, will be versioned separately to preserve reproducibility.

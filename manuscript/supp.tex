...
\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{pareto_closed_status_missing_closed_date_by_agency.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{pareto_nonclosed_status_with_closed_date_by_agency.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{closed_exact_midnight_cy_distribution.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{pareto_closed_exact_midnight_agency.pdf}
...
greater than 730 and at most 1825 days (approximately two to five years) --- duration limits that
seem reasonable to count as \emph{large}. This chosen range also 
excludes \num{1258} extreme durations,
which exceed five years and require separate investigation. There are \num{139713} such \emph{large-duration} SRs.
...
Corporation (EDC) and DSNY.


\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{boxplot_large_positive_739_1825_days_by_agency.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{nypd_vs_others_combined.pdf}
...
A similar situation is illustrated in Figure~\ref{fig:violin-boxplot-negative}
showing negative durations --- where the \texttt{closed\_date} occurs before 
the \texttt{created\_date} resulting in a nonsensical \emph{negative} duration. 
Here the violin plot shows DOT to be the largest offender with \num{44143} 
such SRs. However, the mean negative duration is far greater for the 
Department of Environmental Protection (DEP) as shown on the logarithmic 
x-axis. This violin plot visualization captures both the count, magnitude, 
and the extent of the negative duration anomaly. Box charts of these 
same distributions are also produced and available in 
the \texttt{charts} directory upon completion of the data cleansing program. 
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{violin_boxplot_negative_days_by_agency.pdf}
\caption{Violin with Boxplot for SRs with Negative Durations (log x-axis), by Agency.}
...
Applying the unrounded cutoff identified \num{10231} short durations
(\SI{0.067}{\percent} of positive durations of at least two seconds).
The category summary uses a rounded boundary of at most \SI{28}{\second},
yielding \num{14741} positive durations, including \num{3460} one-second
cases. Together with \num{387329} zero-duration cases, these comprise
\num{402070} SRs (\SI{2.55}{\percent} of closed rows).
...
second, as shown for approximately the first ten minutes of each hour. Further analysis identified
the NYC Department of Environmental Protection (DEP) and 
Department of Sanitation (DSNY) as the largest contributors to the
on-the-minute spikes.

\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{duration_histogram.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{created_date_first_601_seconds.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{annual_backlog_bar_chart.pdf}
...
the \texttt{jds\_datacleansings.R} program, there will be 87 charts in PDF format in
...
DOI: \url{https://doi.org/10.6084/m9.figshare.30699800}
...
\url{https://github.com/tusseyd/nyc_311_data_cleaning/blob/main/reference_console_output/referenc_JDS_datacleaning_console_output.txt}
...
\item \texttt{code/jds\_datacleansings.R}
...
cleansing program \texttt{code/jds\_datacleansings.R}.
...
.4 jds\_datacleansings.R.
.4 functions/.
.5 [function files].
.3 console\_output/.
.4 JDS\_data\_prep\_console\_output.txt.
.4 JDS\_datacleaning\_console\_output.txt.
.3 data/.
...
.4 referenc\_JDS\_datacleaning\_console\_output.txt.
}


Step 3: Open RStudio with the project root as the working directory,
then open \texttt{code/data\_prep\_for\_jds\_datacleansing.R}.
Both scripts set \texttt{base\_dir} from \texttt{getwd()}, so this directory must
be \path{nyc_311_data_cleaning}. Alternatively, set
\texttt{base\_dir} explicitly in both scripts:

\begin{verbatim}
base_dir <- file.path("your", "path", "nyc_311_data_cleaning")
\end{verbatim}
...
\begin{enumerate}
\item Execute \texttt{code/jds\_datacleansings.R} in RStudio.
...
\item The script will produce:
  \begin{itemize}
    ...
    \item \texttt{field\_usage\_summary\_table.csv}, located in \texttt{analytics/}
...
\item Data preparation console output: \\ \small\url{https://github.com/tusseyd/nyc_311_data_cleaning/blob/main/reference_console_output/reference_JDS_data_prep_console_output.txt}
 \item Data cleaning console output: \\ \small\url{https://github.com/tusseyd/nyc_311_data_cleaning/blob/main/reference_console_output/referenc_JDS_datacleaning_console_output.txt}
\end{itemize}

...
\midrule
New York Police Department & NYPD & \SI{43}{\percent} & \SI{43}{\percent} \\
Dept of Housing Preservation \& Development & HPD & \SI{19}{\percent} & \SI{62}{\percent} \\
Dept of Sanitation & DSNY & \SI{12}{\percent} & \SI{74}{\percent} \\
Dept of Transportation & DOT & \SI{7}{\percent} & \SI{81}{\percent} \\
Dept of Environmental Protection & DEP & \SI{5}{\percent} & \SI{86}{\percent} \\
Dept of Parks \& Recreation & DPR & \SI{4}{\percent} & \SI{90}{\percent} \\
\bottomrule
\end{tabular}
\end{table}
...
entries occur more than \SI{30} day post-closure, with some extending to
\SI{1593} days (over four years). Figure~\ref{fig:resolution-violin} 
...
DSNY. In addition, \num{91} records exhibit extremely late
post-closure updates due to default \texttt{closed\_date}
values such as 1899-12-31 or 1900-01-01, which produce
artificially extended post-closure durations. This behavior is consistent 
with default date handling in Structured Query Language 
(SQL), where null date-time values may be coerced to 1900-01-01. 
A similar effect can occur in Microsoft Excel.


\begin{figure}[tbp]
\centering
\includegraphics[width=0.97\textwidth]{post_closed_resolution_violin.pdf}
\caption{Violin plot showing SRs with long post-closure resolution updates. Note the 
...
during processing. When a timestamp is absent and field is converted to a datetime 
...
exceeding expectations and a \(+3\sigma\) threshold indicated by the red line.
...
exactly midnight (00:00:00) and exactly noon (12:00:00). The red line represents 3$\sigma$ deviation.}
...
A total of \num{45296} SRs
(\SI[round-mode=places, round-precision=2]{0.29}{\percent})
have a \texttt{closed\_date} that precedes the
\texttt{created\_date}, producing a nonsensical negative duration. Across 
all affected SRs, negative durations 
average \SI[round-mode=places, round-precision=1]{-118.7}{\day},
representing a substantial temporal distortion.
Within this group, \num{115} SRs exhibit \emph{extreme}
negative durations due to default \texttt{closed\_date} values
(1899-12-31 and 1900-01-01), with durations exceeding
$-122$~years. Those dates are consistent with default SQL for NULL datetime 
field behaviors. These extreme negative durations are concentrated 
within DOT,which accounts for \SI{98}{\percent} of such cases.
...
(\SI{2.5}{\percent}) such zero-duration SRs. In addition, \num{3460} SRs 
are closed within one second of their creation. While not logically 
impossible, these near-instantaneous SR resolutions are statistically improbable.
\SI{81}{\percent} of these cases originate from just three agencies:
...
(\SI{48}{\hour}). The analysis evaluated five complementary outlier screening methods:
...
of \SI{27.95}{\second}, which served as the outlier threshold.
This threshold identified
\SI[round-mode=places, round-precision=2]{0.07}{\percent}
of records as outliers (\num{14741}~SRs) with statistically
anomalous durations. Combining zero-duration cases (\num{387329}),
one-second durations (\num{1240}),
and durations durations less than \SI{28}{\second} (rounded 
from \SI{27.95}{\second}) (\num{14471}), a total of \num{403310}~SRs
(\SI{2.6}{\percent}) were classified as statistically anomalous
or operationally implausible.
...
discussions regarding data standards, formats, and metadata ensure that  NYC
datasets support the needs of policymakers, researchers, and 
the public. Clear communication helps surface recurring 
issues, clarify user requirements, and guide improvements to dataset 
documentation and structure. Accordingly, agencies should appoint a
Data Curation Officer to oversee these efforts.

...
serving as an essential resource for research across domains such as 
...
enrollment trends. These initiatives have enabled significant research across multiple
...
of the NYC 311 Service Request (SR) dataset.
 This dataset serves as a prime example for examining key issues 
 in data curation, including data validity, consistency, and curation 
 efficiency. Examples drawn from the processing of the 311 SR 
 data illustrate these points. Second, this study proposes a 
...
\item Similarly, the dataset contains seven street-related fields, including 
two potentially redundant pairs:
...
\item A free-form text field, \texttt{resolution\_description}, supporting 
\num{930} characters including commas and special 
...
driveway. Figure~\ref{fig:SR_complaints} shows that the top \num{20} \texttt{complaint\_type} 
categories comprise \SI{68}{\percent} of all SRs, with the remaining
\num{190} types distributed across a highly skewed categorical 
distribution. This suggests that improving responses to the most 
frequent complaints could have an outsized impact on overall service efficiency. 

\begin{figure}[tbp]
 \centering
  \includegraphics[width = 0.9\textwidth]{SR_by_complaint_type_pareto_combo_chart.pdf} 
...
agencies, which can facilitate targeted corrective actions. 
Figure~\ref{fig:SRcountbyAgency} shows the cumulative
distribution of SRs by agency over the five-year period.
SRs are concentrated among six agencies:
...
which together handle \SI{90}{\percent} of all SRs. The 
remaining \SI{10}{\percent} are distributed across
\num{10} additional agencies. The concentration of SRs among these ``big six'' agencies 
underscores the necessity for optimal data curation processes within these agencies. 

\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.9\textwidth]{SR_by_agency_pareto_combo_chart.pdf}
...
This study identifies issues in the NYC 311 SR open dataset,
...
data cleansing efforts. The analysis identified three structural issues:
precision versus accuracy discrepancies in the
\texttt{latitude}, \texttt{longitude}, and \texttt{location}
fields; inconsistent representations of missing data; and
systemic daylight saving time anomalies.
...
The 311 SR dataset contains inconsistent representations
of missing data, including nulls, spaces, \texttt{NA},
\texttt{N/A}, and \texttt{<NA>}. This variety of representations 
complicates programming efforts and increases the likelihood 
of introducing errors typically by searching for or excluding 
...
\label{sec:domain}
Any analytic effort must ensure that fields contain valid values
...
\SI{1593}{\day} (over four years). Figure~\ref{fig:resolution-violin} 
...
artificially extended post-closure durations.
 Such behavior is consistent 
with default date handling in Structured Query Language 
(SQL), where null datetime values may be coerced to 1900-01-01. 
...
\caption{\textsc{SR} post-closure updates \SIrange{30}{1095}{\day}}
...
\caption{Negative SR durations greater than one year}
...
fields reveals spikes at exactly midnight (00{:}00{:}00) and noon
...
calculated duration of these SRs as well as overall system usage by providing inaccurate 
timestamps. Distribution by agency indicates that more than
\SI{92}{\percent} of the``closed-exactly-at-midnight'' SRs originate 
...
inflate counts at midnight. However, this does not explain the spikes
...
Figure~\ref{fig:negative-duration-violin} illustrates the
range and volume of these negative durations with a cutoff at
three years. Across all affected SRs, negative durations
...
(1--2~years). These are concentrated in DPR, DOHMH, and the
Economic Development Corporation (\textsc{EDC}),
which together account for \SI{94}{\percent} of such cases. Additionally, 
...
durations occur within the DPR.
...
Building on insights from the NYC 311 SR case study,
this study proposes a set of data curation principles
tailored for government open data. These principles address 
the unique requirements observed in the management of such data, 
helping ensure reliability, consistency, and continued utility 
for public services, research, and related applications. To facilitate 
implementation, the principles are prioritized based on their 
impact and feasibility, offering a clear roadmap for improving data curation practices.

\subsection{Principles}
\label{subsec:principles}

\paragraph{Principle 1: Establishing Cross-Organizational Consistency:}
A central issue in government-released datasets is the harmonization
of data fields across agencies and the maintenance of that consistency
over time. Inconsistent field definitions, data formats, or value
domains can lead to erroneous analyses and hinder 
cross-agency collaboration. To address this, agencies should consider 
...
The Open Data governing authority would then promulgate 
these standards, following agreements by the relevant parties.
...
particularly in high-volume datasets. To the maximum extent 
...
streamline analyses. Reducing file size saves storage space 
and enhances the performance of queries, downloads, and analytical workflows.
...
such an event, reflecting the potential of open data to 
enhance public services. Governments’ support of such 
initiatives not only promotes data literacy but also 
encourages community-driven improvements to datasets, 
...
\begin{itemize}[leftmargin=*, nosep]
  ...
  \item NYC 311 SR dataset archived on Figshare: \url{https://doi.org/10.6084/m9.figshare.28454858}
\end{itemize}
